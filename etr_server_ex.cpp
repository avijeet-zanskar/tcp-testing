...
int main(int argc, char** argv) {
    ...
    do {
        if (int ready_fd = epoll_wait(epoll_fd, events, MAX_EPOLL_EVENTS, 0); ready_fd > 0) {
            for (int i = 0; i < ready_fd; ++i) {
                if (events[i].data.fd == listen_fd and (events[i].events & EPOLLIN)) {
                    ...
                    //int error, value;
                    //ioctl(client_fd, FIONREAD, &value);
                    //std::cout << "unread buffer size = " << value << '\n';
                    ...
                    //int error, value;
                    //ioctl(events[i].data.fd, FIONREAD, &value);
                    //std::cout << "unread buffer size = " << value << '\n';
                    int received_size = 0;
                    static uint64_t read_bytes = 0;
                    //std::cout << "read bytes = " << read_bytes << '\n';
                    //while (received_size < packet_size) {
                    ssize_t size = recv(events[i].data.fd, buf, sizeof(buf), 0); // try MSG_WAITALL
